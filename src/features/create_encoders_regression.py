...
@click.option('--dataset_name', default='train_promotion_reg.parquet', help='Nome do dataset de treino', type=str)
def main(configfile, dataset_name):
    ...
    try:
        feature_fs = yaml.safe_load(open(os.path.join('src', 'features', 'selected', 'features_selected_regression.yaml')))
    except:
    ...
    pickle.dump(
        seletor_1,
        open('models/encoders_reg/seletor_1.pkl', 'wb')
    )

    pickle.dump(
        seletor_2,
        open('models/encoders_reg/seletor_2.pkl', 'wb')
    )

    pickle.dump(
        fill_null,
        open('models/encoders_reg/features_fill_null.pkl', 'wb')
    )

    pickle.dump(
        bool_handler,
        open('models/encoders_reg/bool_handler.pkl', 'wb')
    )

    pickle.dump(
        fill_string_missing,
        open('models/encoders_reg/fill_string_missing.pkl', 'wb')
    )

    pickle.dump(
        normalize_lower_string,
        open('models/encoders_reg/normalize_lower_string.pkl', 'wb')
    )

    if has_grouper:
        pickle.dump(
            grouper,
            open('models/encoders_reg/grouper.pkl', 'wb')
        )

    pickle.dump(
        encoder,
        open('models/encoders_reg/encoder.pkl', 'wb')
    )

    pickle.dump(
        conversor_float,
        open('models/encoders_reg/conversor_float.pkl', 'wb')
    )

    logger.info('Sucesso! Binários salvos')
    logger.info('Salvando a base encodada')

    df['discount'] = target_series

    df.to_parquet(os.path.join('data', 'train_test', 'train_encoded_reg.parquet'), index=False)
