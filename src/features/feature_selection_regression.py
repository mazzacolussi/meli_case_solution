...
def main(configfile, dataset_name):
    ...
    model = RandomForestRegressor(max_depth=6, min_samples_leaf=100, n_jobs=-1, random_state=98)
    ...
    with open(os.path.join('src', 'features', 'selected', 'features_selected_regression.yaml'), 'w') as output:
