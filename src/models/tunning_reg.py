...
from utils.optuna_settings import objective_xgboost_reg
...
@click.option('--dataset_name', default='train_encoded_reg.parquet', help='Nome do dataset de treino encodado', type=str)
def main(configfile, dataset_name):
    ...
    seletor2 = pickle.load(
        open('models/encoders_reg/seletor_2.pkl', 'rb')
    )

    logger.info('Iniciando o Optuna')
    study = optuna.create_study(direction='minimize', sampler=optuna.samplers.TPESampler(seed=12), study_name='XGBoost Regressor')

    func = lambda trial: objective_xgboost_reg(
        trial,
    ...
    with open(os.path.join('models', 'df_metrics_results_tunning_reg.pkl'), 'wb') as output:
